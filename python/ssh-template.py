...
def open_ssh_conn(ip):
    ...
    try:
        ...
        selected_user_file = open("credentails.txt", 'r')
	...
	#        connection = session.invoke_shell()	
        ...
        #Creating a folder
        #connection.send("\n")
        #connection.send("mkdir /Desktop/python/testfolder\n")
        time.sleep(1)
	#OUR CODE
        stdin,stdout,stderr=session.exec_command('mkdir testfolder1')
...
try:
	#if ipv4 addr is valid connect
	socket.inet_aton(ipaddr)
	open_ssh_conn(ipaddr)
	sys.exit()
except socket.error:
	print("Invalid IPv4")
try:
	interface = sys.argv[2]
	#if failed check for v6
	socket.inet_pton(ipaddr,interface)
	open_ssh_conn(ipaddr)
except socket.error:
	print("Invalid IPv6")
#ipaddr=sys.argv[1]
